...
public class ContentController {
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "")
    @PutMapping(value="/{id}")
    public Result update(@RequestBody @ApiParam(name = "Content对象",value = "传入JSON数据",required = false) Content content,@PathVariable String  id){
        //设置主键值
        content.setId(id);
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "")
    @GetMapping("/{id}")
    public Result<Content> findById(@PathVariable String id){
    ...
    public Result<List<Content>> findByCategory(@PathVariable String id){
        //根据分类ID查询广告集合
